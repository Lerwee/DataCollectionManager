...
use app\customs\zapi\common\parsers\CFilterParser;
...
class ItemTestAssist extends BaseAssist
{
    /**
     * Types of preprocessing tests, depending on type of item.
     */
    const PRS_TEST_TYPE_ITEM = 0;
    const PRS_TEST_TYPE_ITEM_PROTOTYPE = 1;
    const PRS_TEST_TYPE_LLD = 2;
     ...
     * @var array
     */
    private static $testable_item_types = [ITEM_TYPE_PERSEUS, ITEM_TYPE_SIMPLE, ITEM_TYPE_INTERNAL, ITEM_TYPE_EXTERNAL,
     ...
     */
    const PRS_DEFAULT_VALUE_TYPE = ITEM_VALUE_TYPE_TEXT;
     ...
     * @var array
     */
    protected $items_support_proxy = [ITEM_TYPE_PERSEUS, ITEM_TYPE_SIMPLE, ITEM_TYPE_INTERNAL, ITEM_TYPE_EXTERNAL,
     ...
     * @var array
     */
    protected $item_types_has_key_mandatory = [ITEM_TYPE_PERSEUS, ITEM_TYPE_SIMPLE, ITEM_TYPE_INTERNAL,
     ...
     * @var array
     */
    protected static $preproc_steps_using_prev_value = [PRS_PREPROC_DELTA_VALUE, PRS_PREPROC_DELTA_SPEED,
        PRS_PREPROC_THROTTLE_VALUE, PRS_PREPROC_THROTTLE_TIMED_VALUE
    ];
...
//            'templateids' => [$hostid]
//        ]);
        $ret = TemplateHelper::getTemplates([
            'countOutput' => true,
            'templateids' => [$hostid]
        ]);
    ...
    protected function getHostProxies()
    {
        ...
        CArrayHelper::sort($proxies, [['field' => 'host', 'order' => PRS_SORT_UP]]);
        ...
        switch ($this->item_type) {
                ...
                if (!array_key_exists('flags', $input)) {
                    ...
                    if ($items) {
                        ...
                        switch ($this->getInput('test_type')) {
                            case self::PRS_TEST_TYPE_LLD:
                                $item_flag = PRS_FLAG_DISCOVERY_RULE;
                                break;

                            case self::PRS_TEST_TYPE_ITEM_PROTOTYPE;
                                $item_flag = PRS_FLAG_DISCOVERY_PROTOTYPE;
                                break;

                            default:
                                $item_flag = PRS_FLAG_DISCOVERY_NORMAL;
                                break;
                        }
                    }
                }
            ...
            case ITEM_TYPE_HTTPAGENT:
                $data += [
                    ...
                    'post_type' => array_key_exists('post_type', $input) ? $input['post_type'] : PRS_POSTTYPE_RAW,
                ...
                if ($data['http_authtype'] != PRS_HTTP_AUTH_NONE) {
                    $data += [
    ...
    protected function getHostInterface(array $inputs) {
        ...
        if (($this->host['status'] == HOST_STATUS_MONITORED || $this->host['status'] == HOST_STATUS_NOT_MONITORED)
            ...
            $output = ['interfaceid', 'hostid', 'type', 'dns', 'ip', 'port', 'main', 'useip'];
            $interfaces = [];
            ...
            $item_type_interface = ItemHelper::itemTypeInterface($this->item_type);
            ...
            if ($item_type_interface === false) {
                $host_interfaces = Interfaces::find()->select($output)
                    ->where(['hostid' => $this->host['hostid']])
                    ->andWhere(['main' => INTERFACE_PRIMARY])
                    ->asArray()->all();
                $host_interfaces = prs_toHash($host_interfaces, 'type');

                foreach (ItemAssist::INTERFACE_TYPES_BY_PRIORITY as $interface_type) {
    ...
    protected function getSupportedMacros(array $inputs) {
        ...
        if (array_key_exists('key', $inputs) && strstr($inputs['key'], '{') !== false) {
            $usermacros = CMacrosResolverHelper::extractItemTestMacros([
                ...
                'support_lldmacros' => ($this->test_type == self::PRS_TEST_TYPE_ITEM_PROTOTYPE),
    ...
    protected function resolvePreprocessingStepMacros(array $steps) {
        ...
        $macros_types = ($this->test_type == self::PRS_TEST_TYPE_ITEM_PROTOTYPE)
        ...
        foreach ($steps as &$step) {
             ...
             * sending data to Perseus server.
             */
            $step['params'] = str_replace("\r\n", "\n", $step['params']);

            // Resolve macros in parameter fields before send data to Perseus server.
    ...
    private function resolveCalcFormulaMacros(string $formula, array $macros_posted): string {
        ...
        $expression_parser = new CExpressionParser([
            'usermacros' => true,
            'lldmacros' => ($this->test_type == self::PRS_TEST_TYPE_ITEM_PROTOTYPE),
            'calculated' => true,
            'host_macro' => true,
            'empty_host' => true
        ]);
    ...
    protected function resolveItemPropertyMacros(array $inputs) {
        ...
        foreach (array_keys($this->macros_by_item_props) as $field) {
            ...
            if ($this->test_type == self::PRS_TEST_TYPE_ITEM_PROTOTYPE) {
    ...
    protected function checkPermissions()
    {
        ...
        if ( $hostid != 0) {
            ...
            if (!$hosts) {
                $hosts = TemplateHelper::getTemplates([
        ...
        if ($this->item_type == ITEM_TYPE_SNMP) {
            if (($interface['details']['version'] == SNMP_V1 || $interface['details']['version'] == SNMP_V2C)
                ...
                error(t('zapi', 'Incorrect value for field "{attribute}", {error}.', [
                    'attribute' =>  t('zapi', 'SNMP community'),
            ...
            if ($interface['details']['version'] == SNMP_V2C || $interface['details']['version'] == SNMP_V3) {
                if (!array_key_exists('max_repetitions', $interface['details'])
                    ...
                    error(t('zapi', 'Incorrect value for field "{attribute}", {error}.', [
                        'attribute' =>  t('zapi', 'Max repetition count'),
                        'error' =>  t('zapi', 'cannot be empty'),
                    ]));
                    return false;
                }
                ...
                if (!is_numeric($interface['details']['max_repetitions'])) {
                    error(t('zapi', 'Incorrect value for field "{attribute}", {error}.', [
                        'attribute' =>  t('zapi', 'Max repetition count'),
                        'error' =>  t('zapi', 'a numeric value is expected'),
                    ]));
                    return false;
                }
                ...
                if ($interface['details']['max_repetitions'] < 1) {
                    error(t('zapi', 'Incorrect value for field "{attribute}", {error}.', [
                        'attribute' =>  t('zapi', 'Max repetition count'),
                        'error' =>  t('zapi', 'value must be no less than "{value}"', ['value' => 1]),
                    ]));
                    return false;
                }

                if ($interface['details']['max_repetitions'] > PRS_MAX_INT32) {
                    error(t('zapi', 'Incorrect value for field "{attribute}", {error}.', [
                        'attribute' =>  t('zapi', 'Max repetition count'),
                        'error' =>  t('zapi', 'value must be no greater than "{value}"', ['value' => PRS_MAX_INT32]),
                    ]));
                    return false;
                }
            }
