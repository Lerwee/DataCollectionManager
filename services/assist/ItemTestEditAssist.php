...
class ItemTestEditAssist extends ItemTestAssist
{
    /**
     ...
     * @return Result
     */
    public function getTestView($params): Result
    {
        try {
    ...
    protected function checkInput()
    {
        ...
        if ($ret) {
            ...
            if ($ret && $steps) {
                $steps = ItemHelper::normalizeItemPreprocessingSteps($steps);
            ...
            elseif ($ret && !$this->is_item_testable) {
                error(t('zapi', 'Test of "{type}" items is not supported.', [
                    'type' => ItemHelper::itemType2str($this->item_type)
                ]));
                $ret = false;
            }
        }
...
//        }
    ...
    protected function getViewData()
    {
        ...
        $preprocessing_steps_input = ItemHelper::normalizeItemPreprocessingSteps($preprocessing_steps_input);
        $preprocessing_steps = [];
        foreach ($preprocessing_steps_input as $preproc) {
            if ($preproc['type'] == PRS_PREPROC_VALIDATE_NOT_SUPPORTED) {
        ...
        $preprocessing_types = prs_objectValues($preprocessing_steps, 'type');
        $preprocessing_names = ItemHelper::getPreprocessingTypes(null, false, $preprocessing_types);
        ...
        $usermacros = CMacrosResolverHelper::extractItemTestMacros([
            ...
            'delay' => $show_prev ? $this->getInput('delay', PRS_ITEM_DELAY_DEFAULT) : '',
        ...
        if (array_key_exists('interface', $inputs)) {
            if (array_key_exists('address', $inputs['interface'])
                && strstr($inputs['interface']['address'], PRS_SECRET_MASK) !== false) {
                $inputs['interface']['address'] = '';
                $show_warning = true;
            }

            if (array_key_exists('port', $inputs['interface']) && $inputs['interface']['port'] === PRS_SECRET_MASK) {
            ...
            if (array_key_exists('details', $inputs['interface'])) {
                foreach ($inputs['interface']['details'] as $field => $value) {
                    if (strstr($value, PRS_SECRET_MASK) !== false) {
        ...
        foreach (array_keys($usermacros['macros']) as $macro_name) {
            ...
            elseif ($usermacros['macros'][$macro_name] === PRS_SECRET_MASK) {
                /*
        ...
        return [
            'title' => t('zapi', 'Test item'),
            'steps' => $preprocessing_steps,
            'value' => array_key_exists('value', $data) ? $data['value'] : '',
            'eol' => array_key_exists('eol', $data) ? (int) $data['eol'] : PRS_EOL_LF,
