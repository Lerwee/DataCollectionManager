...
use app\customs\zapi\common\export\CConfigurationExport;
use app\customs\zapi\common\export\CConfigurationExportBuilder;
...
class ConfigurationService extends BaseService
{
    ...
    {
        $params = json_decode('{"format":"yaml","prettyprint":true,"options":{"templates":{"10699":"10699"}}}', true);
    ...
    private function validateExport(array &$params, bool $with_unlinked_parent_templates = false): void
    {
        $api_input_rules = ['type' => API_OBJECT, 'fields' => [
            'format' => ['type' => API_STRING_UTF8, 'flags' => API_REQUIRED, 'in' => implode(',',
                [CExportWriterFactory::YAML, CExportWriterFactory::XML, CExportWriterFactory::JSON, CExportWriterFactory::RAW])],
        ...
        if (!ValidateHelper::validate($api_input_rules, $params, '/', $error)) {
    ...
    private function exportForce(array $params)
    {
        ...
        if ($export_data === false) {
            self::exception(PRS_API_ERROR_PERMISSIONS, _('No permissions to referred object or it does not exist!'));
        }
    ...
    protected function validateImport(array &$params): void
    {
        $api_input_rules = ['type' => API_OBJECT, 'fields' => [
            'format' => ['type' => API_STRING_UTF8, 'flags' => API_REQUIRED, 'in' => implode(',',
                [CImportReaderFactory::YAML, CImportReaderFactory::XML, CImportReaderFactory::JSON])],
        ...
        if (array_key_exists('maps', $params['rules']) && ($params['rules']['maps']['createMissing'] || $params['rules']['maps']['updateExisting'])) {
            self::exception(PRS_API_ERROR_PARAMETERS, _s('Incorrect value for field "%1$s": %2$s.', 'rules',
