...
class ExportBuilder extends BaseService
{
    const MARKUP = 'lwops_export';
    /**
    ...
    {
        parent::init();
        $this->data['version'] = PERSEUS_EXPORT_VERSION;
    }
    ...
    private static function buildArrayRow(array $rule, array $row, string $tag, string $main_tag)
    {
        ...
        if (!$default_value && !$has_data) {
            if ($is_required) {
                throw new Exception(t('zapi', 'Invalid tag "{tag}": {error}.', [
                    'tag' => $tag,
                    'error' => t('zapi', 'the tag "{tag}" is missing', ['tag' => $tag])
                ]));
            }
            return null;
        }
        ...
        if (array_key_exists('in', $rule)) {
            if (!array_key_exists($value, $rule['in'])) {
                throw new Exception(t('zapi', 'Invalid tag "{tag}": {error}.', [
                    'tag' => $tag,
                    'error' => t('zapi', 'unexpected constant value "{value}"', ['value' => $value])
                ]));
            }
    ...
    {
        $filter['conditions'] = self::sortLldRuleFilterConditions($filter['conditions'], $filter['evaltype']);
    ...
    private static function formatPreprocessingSteps(array $preprocessing_steps)
    {
        foreach ($preprocessing_steps as &$preprocessing_step) {
            $preprocessing_step['parameters'] = ($preprocessing_step['type'] == PRS_PREPROC_SCRIPT)
    ...
    protected function formatGraphs(array $graphs)
    {
        ...
        foreach ($graphs as $graph) {
            ...
            if ($graph['flags'] == PRS_FLAG_DISCOVERY_PROTOTYPE) {
    ...
    protected function formatTriggers(array $triggers)
    {
        ...
        foreach ($triggers as $trigger) {
            ...
            if ($trigger['flags'] == PRS_FLAG_DISCOVERY_PROTOTYPE) {
        ...
        usort($fields, static function (array $widget_field_a, array $widget_field_b): int {
            ...
            foreach ($value_fields as $value_field) {
                $comparison = strnatcasecmp(
                    $widget_field_a['value'][$value_field],
    ...
    {
        return [static::MARKUP => $this->data];
    }
}
