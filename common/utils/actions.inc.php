...
function condition_type2str($type = null) {
	$types = [
		PRS_CONDITION_TYPE_SUPPRESSED => _('Problem is suppressed'),
		PRS_CONDITION_TYPE_EVENT_NAME => _('Event name'),
		PRS_CONDITION_TYPE_TRIGGER_SEVERITY => _('Trigger severity'),
		PRS_CONDITION_TYPE_TRIGGER => _('Trigger'),
		PRS_CONDITION_TYPE_HOST_NAME => _('Host name'),
		PRS_CONDITION_TYPE_HOST_GROUP => _('Host group'),
		PRS_CONDITION_TYPE_TEMPLATE => _('Template'),
		PRS_CONDITION_TYPE_HOST => _('Host'),
		PRS_CONDITION_TYPE_TIME_PERIOD => _('Time period'),
		PRS_CONDITION_TYPE_DRULE => _('Discovery rule'),
		PRS_CONDITION_TYPE_DCHECK => _('Discovery check'),
		PRS_CONDITION_TYPE_DOBJECT => _('Discovery object'),
		PRS_CONDITION_TYPE_DHOST_IP => _('Host IP'),
		PRS_CONDITION_TYPE_DSERVICE_TYPE => _('Service type'),
		PRS_CONDITION_TYPE_DSERVICE_PORT => _('Service port'),
		PRS_CONDITION_TYPE_DSTATUS => _('Discovery status'),
		PRS_CONDITION_TYPE_DUPTIME => _('Uptime/Downtime'),
		PRS_CONDITION_TYPE_DVALUE => _('Received value'),
		PRS_CONDITION_TYPE_EVENT_ACKNOWLEDGED => _('Event acknowledged'),
		PRS_CONDITION_TYPE_PROXY => _('Proxy'),
		PRS_CONDITION_TYPE_EVENT_TYPE => _('Event type'),
		PRS_CONDITION_TYPE_HOST_METADATA => _('Host metadata'),
		PRS_CONDITION_TYPE_EVENT_TAG => _('Tag name'),
		PRS_CONDITION_TYPE_EVENT_TAG_VALUE => _('Tag value'),
		PRS_CONDITION_TYPE_SERVICE => _('Service'),
		PRS_CONDITION_TYPE_SERVICE_NAME => _('Service name')
	];
	...
	foreach ($actions as $i => $action) {
		...
		foreach ($action['filter']['conditions'] as $j => $condition) {
			...
			switch ($condition['conditiontype']) {
				case PRS_CONDITION_TYPE_HOST_GROUP:
					$groupIds[$condition['value']] = $condition['value'];
					break;

				case PRS_CONDITION_TYPE_TRIGGER:
					$triggerIds[$condition['value']] = $condition['value'];
					break;

				case PRS_CONDITION_TYPE_HOST:
					$hostIds[$condition['value']] = $condition['value'];
					break;

				case PRS_CONDITION_TYPE_TEMPLATE:
					$templateIds[$condition['value']] = $condition['value'];
					break;

				case PRS_CONDITION_TYPE_PROXY:
					$proxyIds[$condition['value']] = $condition['value'];
					break;

				case PRS_CONDITION_TYPE_SERVICE:
					$serviceids[$condition['value']] = $condition['value'];
					break;

				// return values as is for following condition types
				case PRS_CONDITION_TYPE_EVENT_NAME:
				case PRS_CONDITION_TYPE_HOST_METADATA:
				case PRS_CONDITION_TYPE_HOST_NAME:
				case PRS_CONDITION_TYPE_TIME_PERIOD:
				case PRS_CONDITION_TYPE_DHOST_IP:
				case PRS_CONDITION_TYPE_DSERVICE_PORT:
				case PRS_CONDITION_TYPE_DUPTIME:
				case PRS_CONDITION_TYPE_DVALUE:
				case PRS_CONDITION_TYPE_EVENT_TAG:
				case PRS_CONDITION_TYPE_EVENT_TAG_VALUE:
				case PRS_CONDITION_TYPE_SERVICE_NAME:
					$result[$i][$j] = $condition['value'];
					break;

				case PRS_CONDITION_TYPE_EVENT_ACKNOWLEDGED:
					$result[$i][$j] = $condition['value'] ? _('Ack') : _('Not Ack');
					break;

				case PRS_CONDITION_TYPE_TRIGGER_SEVERITY:
					$result[$i][$j] = CSeverityHelper::getName((int) $condition['value']);
					break;

				case PRS_CONDITION_TYPE_DRULE:
					$dRuleIds[$condition['value']] = $condition['value'];
					break;

				case PRS_CONDITION_TYPE_DCHECK:
					$dCheckIds[$condition['value']] = $condition['value'];
					break;

				case PRS_CONDITION_TYPE_DOBJECT:
					$result[$i][$j] = discovery_object2str($condition['value']);
					break;

				case PRS_CONDITION_TYPE_DSERVICE_TYPE:
					$result[$i][$j] = discovery_check_type2str($condition['value']);
					break;

				case PRS_CONDITION_TYPE_DSTATUS:
					$result[$i][$j] = discovery_object_status2str($condition['value']);
					break;

				case PRS_CONDITION_TYPE_EVENT_TYPE:
...
function getConditionDescription($condition_type, $operator, $value, $value2) {
	if ($condition_type == PRS_CONDITION_TYPE_EVENT_TAG_VALUE) {
		$description = [_('Value of tag')];
		$description[] = ' ';
		$description[] = italic($value2);
		$description[] = ' ';
	}
	elseif ($condition_type == PRS_CONDITION_TYPE_SUPPRESSED) {
		return ($operator == CONDITION_OPERATOR_YES)
			? [_('Problem is suppressed')]
			: [_('Problem is not suppressed')];
	}
	elseif ($condition_type == PRS_CONDITION_TYPE_EVENT_ACKNOWLEDGED) {
...
function getActionOperationData(array $operations): array {
	...
	if (array_key_exists('scriptids', $data)) {
		$result['scripts'] = API::Script()->get([
			...
			'filter' => ['scope' => PRS_SCRIPT_SCOPE_ACTION],
			'preservekeys' => true
		]);
	}
		...
		switch ($operation['operationtype']) {
			...
			case OPERATION_TYPE_COMMAND:
				...
				if ($eventsource == EVENT_SOURCE_SERVICE) {
					$result[$i][] = [
						bold(_s('Run script "%1$s" on Perseus server', $scripts[$scriptid]['name'])),
						BR()
					];
...
function get_conditions_by_eventsource($eventsource) {
	$conditions[EVENT_SOURCE_TRIGGERS] = [
		PRS_CONDITION_TYPE_EVENT_NAME,
		PRS_CONDITION_TYPE_TRIGGER,
		PRS_CONDITION_TYPE_TRIGGER_SEVERITY,
		PRS_CONDITION_TYPE_HOST,
		PRS_CONDITION_TYPE_HOST_GROUP,
		PRS_CONDITION_TYPE_SUPPRESSED,
		PRS_CONDITION_TYPE_EVENT_TAG,
		PRS_CONDITION_TYPE_EVENT_TAG_VALUE,
		PRS_CONDITION_TYPE_TEMPLATE,
		PRS_CONDITION_TYPE_TIME_PERIOD
	];
	$conditions[EVENT_SOURCE_DISCOVERY] = [
		PRS_CONDITION_TYPE_DHOST_IP,
		PRS_CONDITION_TYPE_DCHECK,
		PRS_CONDITION_TYPE_DOBJECT,
		PRS_CONDITION_TYPE_DRULE,
		PRS_CONDITION_TYPE_DSTATUS,
		PRS_CONDITION_TYPE_PROXY,
		PRS_CONDITION_TYPE_DVALUE,
		PRS_CONDITION_TYPE_DSERVICE_PORT,
		PRS_CONDITION_TYPE_DSERVICE_TYPE,
		PRS_CONDITION_TYPE_DUPTIME
	];
	$conditions[EVENT_SOURCE_AUTOREGISTRATION] = [
		PRS_CONDITION_TYPE_HOST_NAME,
		PRS_CONDITION_TYPE_HOST_METADATA,
		PRS_CONDITION_TYPE_PROXY
	];
	$conditions[EVENT_SOURCE_INTERNAL] = [
		PRS_CONDITION_TYPE_EVENT_TYPE,
		PRS_CONDITION_TYPE_HOST,
		PRS_CONDITION_TYPE_HOST_GROUP,
		PRS_CONDITION_TYPE_EVENT_TAG,
		PRS_CONDITION_TYPE_EVENT_TAG_VALUE,
	...
	$conditions[EVENT_SOURCE_SERVICE] = [
		PRS_CONDITION_TYPE_SERVICE,
		PRS_CONDITION_TYPE_SERVICE_NAME,
		PRS_CONDITION_TYPE_EVENT_TAG,
		PRS_CONDITION_TYPE_EVENT_TAG_VALUE
	];
...
function get_opconditions_by_eventsource($eventsource) {
	$conditions = [
		EVENT_SOURCE_TRIGGERS => [PRS_CONDITION_TYPE_EVENT_ACKNOWLEDGED],
...
function get_operators_by_conditiontype($conditiontype) {
	$operators[PRS_CONDITION_TYPE_HOST_GROUP] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_TEMPLATE] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_HOST] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_TRIGGER] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_EVENT_NAME] = [
		CONDITION_OPERATOR_LIKE,
		CONDITION_OPERATOR_NOT_LIKE
	];
	$operators[PRS_CONDITION_TYPE_TRIGGER_SEVERITY] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL,
		CONDITION_OPERATOR_MORE_EQUAL,
		CONDITION_OPERATOR_LESS_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_TIME_PERIOD] = [
		CONDITION_OPERATOR_IN,
		CONDITION_OPERATOR_NOT_IN
	];
	$operators[PRS_CONDITION_TYPE_SUPPRESSED] = [
		CONDITION_OPERATOR_NO,
		CONDITION_OPERATOR_YES
	];
	$operators[PRS_CONDITION_TYPE_DRULE] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_DCHECK] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_DOBJECT] = [
		CONDITION_OPERATOR_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_PROXY] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_DHOST_IP] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_DSERVICE_TYPE] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_DSERVICE_PORT] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_DSTATUS] = [
		CONDITION_OPERATOR_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_DUPTIME] = [
		CONDITION_OPERATOR_MORE_EQUAL,
		CONDITION_OPERATOR_LESS_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_DVALUE] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL,
		CONDITION_OPERATOR_MORE_EQUAL,
		CONDITION_OPERATOR_LESS_EQUAL,
	...
	$operators[PRS_CONDITION_TYPE_EVENT_ACKNOWLEDGED] = [
		CONDITION_OPERATOR_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_HOST_NAME] = [
		CONDITION_OPERATOR_LIKE,
		CONDITION_OPERATOR_NOT_LIKE,
		CONDITION_OPERATOR_REGEXP,
		CONDITION_OPERATOR_NOT_REGEXP
	];
	$operators[PRS_CONDITION_TYPE_EVENT_TYPE] = [
		CONDITION_OPERATOR_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_HOST_METADATA] = [
		CONDITION_OPERATOR_LIKE,
		CONDITION_OPERATOR_NOT_LIKE,
		CONDITION_OPERATOR_REGEXP,
		CONDITION_OPERATOR_NOT_REGEXP
	];
	$operators[PRS_CONDITION_TYPE_EVENT_TAG] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL,
		CONDITION_OPERATOR_LIKE,
		CONDITION_OPERATOR_NOT_LIKE
	];
	$operators[PRS_CONDITION_TYPE_EVENT_TAG_VALUE] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL,
		CONDITION_OPERATOR_LIKE,
		CONDITION_OPERATOR_NOT_LIKE
	];
	$operators[PRS_CONDITION_TYPE_SERVICE] = [
		CONDITION_OPERATOR_EQUAL,
		CONDITION_OPERATOR_NOT_EQUAL
	];
	$operators[PRS_CONDITION_TYPE_SERVICE_NAME] = [
		CONDITION_OPERATOR_LIKE,
		CONDITION_OPERATOR_NOT_LIKE
	];
...
function getEventsSuppressions(array $events) {
	...
	foreach ($events as $event) {
		...
		foreach ($event['acknowledges'] as $ack) {
			if (($ack['action'] & PRS_PROBLEM_UPDATE_SUPPRESS) == PRS_PROBLEM_UPDATE_SUPPRESS) {
				$event_suppressions[] = [
			...
			elseif (($ack['action'] & PRS_PROBLEM_UPDATE_UNSUPPRESS) == PRS_PROBLEM_UPDATE_UNSUPPRESS) {
				$event_suppressions[] = [
					'userid' => $ack['userid'],
					'clock' => $ack['clock']
				];
		...
		CArrayHelper::sort($event_suppressions, [['field' => 'clock', 'order' => PRS_SORT_DOWN]]);
...
function getEventsMessages(array $events) {
	...
	foreach ($events as $event) {
		...
		foreach ($event['acknowledges'] as $ack) {
			if (($ack['action'] & PRS_PROBLEM_UPDATE_MESSAGE) == PRS_PROBLEM_UPDATE_MESSAGE) {
				$event_messages[] = [
					'message' => $ack['message'],
					'userid' => $ack['userid'],
					'clock' => $ack['clock']
		...
		CArrayHelper::sort($event_messages, [['field' => 'clock', 'order' => PRS_SORT_DOWN]]);
...
function getEventsSeverityChanges(array $events, array $triggers) {
	...
	foreach ($events as $event) {
		...
		foreach ($event['acknowledges'] as $ack) {
			if (($ack['action'] & PRS_PROBLEM_UPDATE_SEVERITY) == PRS_PROBLEM_UPDATE_SEVERITY) {
				$event_severities[] = [
		...
		CArrayHelper::sort($event_severities, [['field' => 'clock', 'order' => PRS_SORT_DOWN]]);
...
function getSingleEventActions(array $event, array $r_events, array $alerts) {
	...
	$actions[] = [
		'action_type' => PRS_EVENT_HISTORY_PROBLEM_EVENT,
		'clock' => $event['clock']
	];
	...
	if (array_key_exists($event['r_eventid'], $r_events)) {
		$actions[] = [
			'action_type' => PRS_EVENT_HISTORY_RECOVERY_EVENT,
	...
	foreach ($event['acknowledges'] as $ack) {
		$ack['action_type'] = PRS_EVENT_HISTORY_MANUAL_UPDATE;
		$actions[] = $ack;
	...
	foreach ($alerts as $alert) {
		...
		if (bccomp($alert['eventid'], $event['eventid']) == 0
			...
			$alert['action_type'] = PRS_EVENT_HISTORY_ALERT;
			$actions[] = $alert;
	...
	CArrayHelper::sort($actions, [
		['field' => 'clock', 'order' => PRS_SORT_DOWN],
		['field' => 'action_type', 'order' => PRS_SORT_DOWN],
		['field' => 'alertid', 'order' => PRS_SORT_DOWN]
	]);
...
function getEventUpdates(array $event) {
	...
	CArrayHelper::sort($event['acknowledges'], [['field' => 'clock', 'order' => PRS_SORT_DOWN]]);
...
function makeEventActionsIcons($eventid, $actions, $users, $is_acknowledged) {
	...
	if ($is_acknowledged) {
		$action_icons[] = makeActionIcon(['icon' => PRS_STYLE_ACTION_ICON_ACK_GREEN, 'title' => _('Acknowledged')]);
	}
	...
	return $action_icons ? (new CCol($action_icons))->addClass(PRS_STYLE_NOWRAP) : '';
}
...
function makeEventSuppressionsProblemIcon(array $data, array $users): ?CButton {
	...
	for ($i = 0; $i < $total && $i < PRS_WIDGET_ROWS; $i++) {
		$suppression = $data['suppress_until'][$i];

		// Added in order to reuse makeActionTableUser().
		$suppression['action_type'] = PRS_EVENT_HISTORY_MANUAL_UPDATE;

		if (array_key_exists('suppress_until', $suppression)) {
			$icon = PRS_STYLE_ACTION_ICON_SUPPRESS;
			$title = _('Suppressed');

			if ($suppression['suppress_until'] == PRS_PROBLEM_SUPPRESS_TIME_INDEFINITE) {
				...
				$suppress_until = $suppression['suppress_until'] < strtotime('tomorrow')
					...
					? prs_date2str(TIME_FORMAT, $suppression['suppress_until'])
					: prs_date2str(DATE_TIME_FORMAT, $suppression['suppress_until']);
			}
		}
		else {
			$icon = PRS_STYLE_ACTION_ICON_UNSUPPRESS;
			$title = _('Unsuppressed');
			$suppress_until = '';
		}

		$table->addRow([
			prs_date2str(DATE_TIME_FORMAT_SECONDS, $suppression['clock']),
	...
	return $total
		? makeActionIcon([
			'icon' => array_key_exists('suppress_until', $data['suppress_until'][0])
				? PRS_STYLE_ACTION_ICON_SUPPRESS
				: PRS_STYLE_ACTION_ICON_UNSUPPRESS,
			'button' => true,
			'hint' => [
				$table,
				($total > PRS_WIDGET_ROWS)
					? (new CDiv(
						(new CDiv(
							(new CDiv(_s('Displaying %1$s of %2$s found', PRS_WIDGET_ROWS, $total)))
								->addClass(PRS_STYLE_TABLE_STATS)
						))->addClass(PRS_STYLE_PAGING_BTN_CONTAINER)
					))->addClass(PRS_STYLE_TABLE_PAGING)
					: null
			]
		])
		: null;
...
function makeEventMessagesIcon(array $data, array $users): ?CButton {
	...
	for ($i = 0; $i < $total && $i < PRS_WIDGET_ROWS; $i++) {
		...
		// Added in order to reuse makeActionTableUser().
		$message['action_type'] = PRS_EVENT_HISTORY_MANUAL_UPDATE;

		$table->addRow([
			prs_date2str(DATE_TIME_FORMAT_SECONDS, $message['clock']),
			makeActionTableUser($message, $users),
			prs_nl2br($message['message'])
		]);
	}

	return $total
		? makeActionIcon([
			'icon' => PRS_STYLE_ACTION_ICON_MSGS,
			'button' => true,
			'hint' => [
				$table,
				($total > PRS_WIDGET_ROWS)
...
function makeEventSeverityChangesIcon(array $data, array $users): ?CButton {
	...
	for ($i = 0; $i < $total && $i < PRS_WIDGET_ROWS; $i++) {
		...
		// Added in order to reuse makeActionTableUser().
		$severity['action_type'] = PRS_EVENT_HISTORY_MANUAL_UPDATE;
		...
		$table->addRow([
			prs_date2str(DATE_TIME_FORMAT_SECONDS, $severity['clock']),
 ...
 * @param int    $actions[]['action_type']     Type of action table entry (PRS_EVENT_HISTORY_*).
 ...
 * @param string $actions[]['action']          Flag with problem update operation performed (only for PRS_EVENT_HISTORY_MANUAL_UPDATE).
 * @param string $actions[]['alerttype']       Type of alert (only for PRS_EVENT_HISTORY_ALERT).
 * @param string $actions[]['mediatypeid']     Id for mediatype, where alert message was sent (only for PRS_EVENT_HISTORY_ALERT).
 * @param string $actions[]['error']           Error message in case of failed alert (only for PRS_EVENT_HISTORY_ALERT).
 * @param array  $users                        User name, surname and username.
...
function makeEventActionsTable(array $actions, array $users, array $mediatypes): CTableInfo {
	...
	for ($i = 0; $i < $action_count && $i < PRS_WIDGET_ROWS; $i++) {
		$action = $actions[$i];

		$message = '';
		if ($action['action_type'] == PRS_EVENT_HISTORY_MANUAL_UPDATE
				&& ($action['action'] & PRS_PROBLEM_UPDATE_MESSAGE) == PRS_PROBLEM_UPDATE_MESSAGE) {
			$message = prs_nl2br($action['message']);
		}
		elseif ($action['action_type'] == PRS_EVENT_HISTORY_ALERT) {
		...
		$table->addRow([
			prs_date2str(DATE_TIME_FORMAT_SECONDS, $action['clock']),
...
function makeEventActionsIcon(array $data, $eventid): ?CButton {
	...
	if ($data['has_failed_action']) {
		$icon_style = PRS_STYLE_ACTIONS_NUM_RED;
	}
	elseif ($data['has_uncomplete_action']) {
		$icon_style = PRS_STYLE_ACTIONS_NUM_YELLOW;
	}
	else {
		$icon_style = PRS_STYLE_ACTIONS_NUM_GRAY;
	}
 ...
 * @param int    $data['actions'][]['action_type']    Type of action table entry (PRS_EVENT_HISTORY_*).
 ...
 * @param string $data['actions'][]['alerttype']      Type of alert (only for PRS_EVENT_HISTORY_ALERT).
 * @param string $data['actions'][]['esc_step']       Alert escalation step (only for PRS_EVENT_HISTORY_ALERT).
 * @param string $data['actions'][]['subject']        Message alert subject (only for PRS_EVENT_HISTORY_ALERT).
 * @param string $data['actions'][]['p_eventid']      Problem eventid that was reason for alert (only for PRS_EVENT_HISTORY_ALERT).
 * @param string $data['actions'][]['acknowledgeid']  Problem update action that was reason for alert (only for PRS_EVENT_HISTORY_ALERT).
 * @param array  $users                               User name, surname and username.
...
function makeEventDetailsActionsTable(array $data, array $users, array $mediatypes) {
	...
	foreach ($data['actions'] as $action) {
		...
		if ($action['action_type'] == PRS_EVENT_HISTORY_ALERT && $action['p_eventid'] == 0
		...
		switch ($action['action_type']) {
			case PRS_EVENT_HISTORY_ALERT:
				switch ($action['alerttype']) {
					case ALERT_TYPE_MESSAGE:
						$message = [bold($action['subject']), BR(), BR(), prs_nl2br($action['message'])];
						break;

					case ALERT_TYPE_COMMAND:
						$message = [bold(_('Command').':'), BR(), prs_nl2br($action['message'])];
						break;
				}
				break;

			case PRS_EVENT_HISTORY_MANUAL_UPDATE:
				$message = prs_nl2br($action['message']);
				break;
		}
...
function makeEventHistoryTable(array $actions, array $users) {
	...
	foreach ($actions as $action) {
		...
		$action['action_type'] = PRS_EVENT_HISTORY_MANUAL_UPDATE;

		$table->addRow([
			prs_date2str(DATE_TIME_FORMAT_SECONDS, $action['clock']),
			makeActionTableUser($action, $users),
			makeActionTableIcon($action),
			(new CCol(prs_nl2br($action['message'])))->addClass(PRS_STYLE_TABLE_FORMS_OVERFLOW_BREAK)
		]);
	}
 ...
 * @param int    $action['action_type']  Type of event table action (PRS_EVENT_HISTORY_*).
...
function makeActionTableUser(array $action, array $users) {
	if (($action['action_type'] == PRS_EVENT_HISTORY_ALERT && $action['alerttype'] == ALERT_TYPE_MESSAGE)
			|| $action['action_type'] == PRS_EVENT_HISTORY_MANUAL_UPDATE) {
...
function makeEventDetailsTableUser(array $action, array $users) {
	if ($action['action_type'] == PRS_EVENT_HISTORY_ALERT && $action['alerttype'] == ALERT_TYPE_MESSAGE) {
		return array_key_exists($action['userid'], $users)
			? [getUserFullname($users[$action['userid']]), BR(), italic(prs_nl2br($action['sendto']))]
			: _('Inaccessible user');
	}
	elseif ($action['action_type'] == PRS_EVENT_HISTORY_MANUAL_UPDATE) {
		return array_key_exists($action['userid'], $users)
			? getUserFullname($users[$action['userid']])
			: _('Inaccessible user');
	}
 ...
 * @param int    $action['action_type']   Type of action table entry (PRS_EVENT_HISTORY_*).
 * @param int    $action['action']        Flag with problem update operation performed. (only for PRS_EVENT_HISTORY_MANUAL_UPDATE)
 * @param int    $action['old_severity']  Severity before problem update. (only for PRS_EVENT_HISTORY_MANUAL_UPDATE)
 * @param int    $action['new_severity']  Severity after problem update. (only for PRS_EVENT_HISTORY_MANUAL_UPDATE)
 * @param int    $action['alerttype']     Type of alert. (only for PRS_EVENT_HISTORY_ALERT)
 *
 * @return CSpan
 */
function makeActionTableIcon(array $action) {
	switch ($action['action_type']) {
		case PRS_EVENT_HISTORY_PROBLEM_EVENT:
			return makeActionIcon(['icon' => PRS_STYLE_PROBLEM_GENERATED, 'title' => _('Problem created')]);

		case PRS_EVENT_HISTORY_RECOVERY_EVENT:
			return makeActionIcon(['icon' => PRS_STYLE_PROBLEM_RECOVERY, 'title' => _('Problem resolved')]);

		case PRS_EVENT_HISTORY_MANUAL_UPDATE:
			$action_icons = [];

			if (($action['action'] & PRS_PROBLEM_UPDATE_CLOSE) == PRS_PROBLEM_UPDATE_CLOSE) {
				$action_icons[] = makeActionIcon([
					'icon' => PRS_STYLE_ACTION_ICON_CLOSE,
			...
			if (($action['action']
				...
				$action_icons[] = makeActionIcon(['icon' => PRS_STYLE_ACTION_ICON_CAUSE, 'title' => _('Cause')]);
			}

			if (($action['action']
					& PRS_PROBLEM_UPDATE_RANK_TO_SYMPTOM) == PRS_PROBLEM_UPDATE_RANK_TO_SYMPTOM) {
				$action_icons[] = makeActionIcon(['icon' => PRS_STYLE_ACTION_ICON_SYMPTOM, 'title' => _('Symptom')]);
			}

			if (($action['action'] & PRS_PROBLEM_UPDATE_ACKNOWLEDGE) == PRS_PROBLEM_UPDATE_ACKNOWLEDGE) {
				$action_icons[] = makeActionIcon(['icon' => PRS_STYLE_ACTION_ICON_ACK, 'title' => _('Acknowledged')]);
			}

			if (($action['action'] & PRS_PROBLEM_UPDATE_UNACKNOWLEDGE) == PRS_PROBLEM_UPDATE_UNACKNOWLEDGE) {
				$action_icons[] = makeActionIcon(['icon' => PRS_STYLE_ACTION_ICON_UNACK,
					'title' => _('Unacknowledged')
				]);
			}

			if (($action['action'] & PRS_PROBLEM_UPDATE_SUPPRESS) == PRS_PROBLEM_UPDATE_SUPPRESS) {
				if ($action['suppress_until'] == PRS_PROBLEM_SUPPRESS_TIME_INDEFINITE) {
					$suppress_until = _s('Indefinitely');
				}
				else {
					$suppress_until = $action['suppress_until'] < strtotime('tomorrow')
							&& $action['suppress_until'] > strtotime('today')
						? prs_date2str(TIME_FORMAT, $action['suppress_until'])
						: prs_date2str(DATE_TIME_FORMAT, $action['suppress_until']);
				}
			...
			if (($action['action'] & PRS_PROBLEM_UPDATE_UNSUPPRESS) == PRS_PROBLEM_UPDATE_UNSUPPRESS) {
				$action_icons[] = makeActionIcon([
					'icon' => PRS_STYLE_ACTION_ICON_UNSUPPRESS,
					'title' => _('Unsuppressed')
				]);
			...
			if (($action['action'] & PRS_PROBLEM_UPDATE_MESSAGE) == PRS_PROBLEM_UPDATE_MESSAGE) {
				$action_icons[] = makeActionIcon(['icon' => PRS_STYLE_ACTION_ICON_MSG, 'title' => _('Message')]);
			}

			if (($action['action'] & PRS_PROBLEM_UPDATE_SEVERITY) == PRS_PROBLEM_UPDATE_SEVERITY) {
				$action_type = ($action['new_severity'] > $action['old_severity'])
					? PRS_STYLE_ACTION_ICON_SEV_UP
					: PRS_STYLE_ACTION_ICON_SEV_DOWN;
			...
			return (new CCol($action_icons))->addClass(PRS_STYLE_NOWRAP);

		case PRS_EVENT_HISTORY_ALERT:
			$action_icon = ($action['alerttype'] == ALERT_TYPE_COMMAND)
					? PRS_STYLE_ACTION_COMMAND
					: PRS_STYLE_ACTION_MESSAGE;
