 ...
 *
 * Examples:
 * _n('%2$s item on host %1$s', '%2$s items on host %1$s', 'Perseus server', 1) // 1 item on host Perseus server
 * _n('%2$s item on host %1$s', '%2$s items on host %1$s', 'Perseus server', 2) // 2 items on host Perseus server
 *
...
function setupLocale(?string $language, ?string &$error = ''): bool
{
    ...
    $locale_variants = $language === null ? prs_locale_variants(PRS_DEFAULT_LANG) : prs_locale_variants($language);
    $locale_set = false;
    $error = '';
    ...
    if (!$locale_set && strtolower($language) !== 'en_gb') {
        ...
        $error = 'Locale for language "' . $language . '" is not found on the web server. Tried to set: ' .
            implode(', ', $locale_variants) . '. Unable to translate Perseus interface.';
    }
