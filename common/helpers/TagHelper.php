...
class TagHelper
{
     ...
     * @return string
     */
    public static function setWhereCondition(array $tags, $evaltype, $parent_alias, $table, $field): string
    {
        $values_by_tag = [];
            ...
            switch ($operator) {
                ...
                case TAG_OPERATOR_NOT_LIKE:
                    ...
                    $values_by_tag[$tag['tag']][$prefix]['value'][]
                        = 'UPPER(' . $table . '.value) LIKE ' . ZSqlHelper::zbxDbstr($value) . " ESCAPE '!'";
                    break;
                ...
                case TAG_OPERATOR_NOT_EQUAL:
                    $values_by_tag[$tag['tag']][$prefix]['value'][] = $table . '.value=' . ZSqlHelper::zbxDbstr($value);
                    break;
            }
        }
        ...
        foreach ($values_by_tag as $tag => $filters) {
            ...
            if (
                array_key_exists('EXISTS', $filters) && array_key_exists('tag', $filters['EXISTS'])
            ...
            foreach ($filters as $prefix => $filter) {
                $statement_start = $prefix . ' (' .
                    ...
                    ' AND ' . $table . '.tag=' . ZSqlHelper::zbxDbstr($tag);
     ...
     * @return string
     */
    public static function setInheritedHostTagsWhereCondition(array $tags, int $evaltype): string
    {
        ...
        $query->where(' ht.hostid=h.hostid')
            ...
            ->andWhere(self::setWhereCondition($swapped_filter, TAG_EVAL_TYPE_OR, 'h', 'host_tag', 'hostid'));
        $query->select(['h.hostid', 'ht.tag', 'ht.value']);
        $db_template_tags = $query->all();
        ...
        foreach ($tags as $tag) {
            ...
            if (
                in_array($tag['operator'], [TAG_OPERATOR_LIKE, TAG_OPERATOR_EQUAL, TAG_OPERATOR_EXISTS])
                && !array_key_exists($tag['tag'], $inclusive_tags)
            ) {
                $inclusive_tags[$tag['tag']] = [];
            } elseif (
                in_array($tag['operator'], [TAG_OPERATOR_NOT_LIKE, TAG_OPERATOR_NOT_EQUAL, TAG_OPERATOR_NOT_EXISTS])
        ...
        foreach ($negated_conditions as $tag => $tag_where) {
            ...
            while ($tag_where['templateids']) {
                ...
                $tag_where['templateids'] = TemplateHelper::getTemplates([
                    'output' => [],
            ...
            $negated_where_conditions[] = '(NOT EXISTS (' .
                ...
                ' AND host_tag.tag=' . ZSqlHelper::zbxDbstr($tag) .
                ($tag_where['values'] ? ' AND (' . implode(' OR ', $tag_where['values']) . ')' : '') .
                ')' .
                ($templateids_in
                    ? ' OR ' . ZSqlHelper::dbConditionInt('ht2.templateid', array_keys($templateids_in)) . ''
                    : ''
                ) .
                ')' .
                ')';
        ...
        foreach ($inclusive_tags as $tag_name => $tag_values) {
            ...
            if ($tag_values === false) {
                $templateids += self::getMatchingTemplateids(
                    ['tag' => $tag_name, 'operator' => TAG_OPERATOR_EXISTS],
                    $db_template_tags
                );
            } else {
            ...
            while ($templateids) {
                ...
                $templateids = TemplateHelper::getTemplates([
                    'output' => [],
            ...
            $where_conditions[] = '(EXISTS (' .
                ...
                ' FROM host_tag' .
                ' WHERE h.hostid=host_tag.hostid' .
                ' AND host_tag.tag=' . ZSqlHelper::zbxDbstr($tag_name) .
                ($values ? ' AND (' . implode(' OR ', $values) . ')' : '') .
                ')' .
                ($templateids_in ? ' OR ' . ZSqlHelper::dbConditionInt('ht2.templateid', array_keys($templateids_in)) : '') .
                ')';
        }
    ...
    private static function makeTagWhereCondition(array $tag): string
    {
        if ($tag['operator'] == TAG_OPERATOR_EQUAL || $tag['operator'] == TAG_OPERATOR_NOT_EQUAL) {
            return 'host_tag.value=' . ZSqlHelper::zbxDbstr($tag['value']);
        } else {
            $value = str_replace(['!', '%', '_'], ['!!', '!%', '!_'], $tag['value']);
            $value = '%' . mb_strtoupper($value) . '%';
            return 'UPPER(host_tag.value) LIKE ' . ZSqlHelper::zbxDbstr($value) . " ESCAPE '!'";
        }
    }
