...
class CArrayHelper {
	...
	public static function sort(array &$array, array $fields) {
		foreach ($fields as $fid => $field) {
			if (!is_array($field)) {
				$fields[$fid] = ['field' => $field, 'order' => PRS_SORT_UP];
			}
		}
		self::$fields = $fields;
	...
	protected static function compare($a, $b) {
		foreach (self::$fields as $field) {
			...
			if ($cmp != 0) {
				return $cmp * ($field['order'] == PRS_SORT_UP?1:-1);
			}
		}
		return 0;
	}
	 ...
	 * Example 2:
	 * $data = array(
	 *     array('host' => 'Perseus server', 'name' => 'CPU load'),
	 * 	   array('host' => 'Perseus server', 'name' => 'Free memory'),
