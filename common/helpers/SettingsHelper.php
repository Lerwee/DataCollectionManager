...
class SettingsHelper
{
    ...
    public static function get(string $field): string
    {
        if (!self::$params) {
            ...
            self::$params = (new Query())->from('config')
                ->select($output_fields)->one();

            if (!self::$params) {
                throw new \Exception('Unable to load settings API parameters.');
            }
        }
    ...
    public static function getPublic(string $field): string
    {
        if (!self::$params_public) {
            self::$params_public = Settings::getPublic();
        }
    ...
    public static function getPrivate(string $field): string
    {
        if (!self::$params_private) {
            self::$params_private = Settings::getPrivate();
        }
