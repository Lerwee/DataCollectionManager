...
class CSettingsHelper {
	...
	public static function get(string $field): string {
		if (!self::$params) {
            ...
            self::$params = Config::find()->select($columns)->asArray()->one();
			if (empty(self::$params)) {
				throw new \Exception(t('zapi', 'Unable to load settings API parameters.'));
			}
		}
	...
	public static function getPublic(string $field): string {
		if (!self::$params_public) {
            ...
            self::$params_public = Config::find()->select($output_fields)->asArray()->one();
		}
	...
	public static function getPrivate(string $field): string {
		if (!self::$params_private) {
            ...
            $data = Config::find()->select($output_fields)->asArray()->one();
            $data['dbversion_status'] = json_decode($data['dbversion_status'], true) ?: [];
            self::$params_private = $data;
		}
