...
class C10TriggerExpression {
	...
	/**
	 * An array of trigger functions like {Perseus server:agent.ping.last(0)}
	 ...
	 * Examples:
	 *   expression:
	 *     {Perseus server:agent.ping.last(0)}=1 and {TRIGGER.VALUE}={$TRIGGER.VALUE}
	 *   results:
	 *     $this->isValid : true
	 *     $this->error : ''
	 *     $this->expressions : array(
	 *       0 => array(
	 *         'expression' => '{Perseus server:agent.ping.last(0)}',
	 *         'pos' => 0,
	 *         'host' => 'Perseus server',
	 *         'item' => 'agent.ping',
	 *         'function' => 'last(0)',
	...
	public function parse($expression) {
		...
		if ($this->pos == 0) {
			$this->error = $this->options['calculated']
				? t('zapi', 'incorrect calculated item formula')
				: t('zapi', 'Incorrect trigger expression.');
			$this->isValid = false;
		}
		...
		if ($error) {
			...
			$this->error = $this->options['calculated']
				? t('zapi', 'incorrect calculated item formula starting from "{pos}"', ['pos' => $exp_part])
				: t('zapi', 'Incorrect trigger expression.').' '.t('zapi', 'Check expression part starting from "{pos}".', ['pos' => $exp_part]);
	...
	public function getHosts() {
		...
		return array_unique(prs_objectValues($this->expressions, 'host'));
	}
