	...
	*/
	...
	public function __construct(array $options = []) {
		...
		if ($this->options['with_size_suffix']) {
			$this->suffixes .= PRS_SIZE_SUFFIXES;

			$this->suffix_multipliers += $this->options['is_binary_size']
				? PRS_SIZE_SUFFIX_MULTIPLIERS_BINARY
				: PRS_SIZE_SUFFIX_MULTIPLIERS;
		}

		if ($this->options['with_time_suffix']) {
			$this->suffixes .= $this->options['with_year'] ? PRS_TIME_SUFFIXES_WITH_YEAR : PRS_TIME_SUFFIXES;
			$this->suffix_multipliers += PRS_TIME_SUFFIX_MULTIPLIERS;
		}
	}
	...
	public function parse($source, $pos = 0): int {
		...
		$pattern = $this->options['with_float'] ? PRS_PREG_NUMBER : PRS_PREG_INT;
