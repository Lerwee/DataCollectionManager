<?php
namespace app\customs\zapi\common\export;
...
class CConfigurationExport {
	...
	protected function filterOptions(array $options) {
		if ($options['hosts']) {
			...
			$hosts = Hosts::find()->select('hostid')->andWhere(['flags'=>PRS_FLAG_DISCOVERY_NORMAL])
                ->column();
			$options['hosts'] = $hosts;
		}
	...
	protected function gatherTemplates(array $templateids) {
        ...
        $searcher->assignable = ['hostid','host', 'name', 'description', 'uuid', 'vendor_name', 'vendor_version'];
        $searcher->isPage = false;
        $dataProvider = $searcher->search(['templateids'=>$templateids]);
        $templates  =  $searcher->format($dataProvider->getModels());
        ...
        //"{"10699":{"host":"create template OS","name":"模板-[OS]","description":"test create os template","uuid":"8c65790b5026455f9ec36dd2c46d7d0d","vendor_name":"","vendor_version":"","templateid":"10699","macros":[],"parentTemplates":[],"valuemaps":[],"templategroups":[{"groupid":"50","name":"Template\/OS","uuid":"e179682f1aea498f9e87764eb7eca886"},{"groupid":"49","name":"Template\/OS\/linux","uuid":"a3b55c80cd0a4364983b4ae360cd8678"}],"dashboards":[],"tags":[]}}"
	...
	protected function gatherProxies(array $hosts) {
		...
		if ($proxy_hostids) {
			...
			$db_proxies = prs_toHash($db_proxies, 'hostid');
		}
