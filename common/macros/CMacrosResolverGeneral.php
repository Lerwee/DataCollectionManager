<?php
namespace app\customs\zapi\common\macros;

use app\customs\zapi\common\helpers\SettingsHelper;
use app\customs\zapi\common\helpers\CSeverityHelper;
...
use app\customs\zapi\common\parsers\CFunctionIdParser;
...
use app\customs\zapi\common\parsers\CMacroFunctionParser;
use app\customs\zapi\common\parsers\CMacroParser;
use app\customs\zapi\common\parsers\CNumberParser;
use app\customs\zapi\common\parsers\CParser;
use app\customs\zapi\common\parsers\CReferenceParser;
use app\customs\zapi\common\parsers\CReplacementParser;
...
use app\modules\libzbx\api\models\Usermacro;
use app\modules\libzbx\helpers\HistoryHelper;
...
class CMacrosResolverGeneral {
	...
	private static function resolveItemKeyParamsMacros($key_chain, array $params_raw, array $macros, array $types) {
		foreach (array_reverse($params_raw) as $param_raw) {
			...
			switch ($param_raw['type']) {
				...
				case CItemKey::PARAM_UNQUOTED:
					$param = ItemHelper::quoteItemKeyParam(strtr($param, $macros), $forced);
					break;
			}
		...
		if ($hist_function_parser->parse($function) == CParser::PARSE_SUCCESS) {
			foreach (array_reverse($hist_function_parser->getParameters()) as $parameter) {
				...
				if ($parameter['type'] != CHistFunctionParser::PARAM_TYPE_PERIOD) {
					$param = TriggerHelper::quoteFunctionParam($param, $forced);
				}
	...
	protected function getIpMacros(array $macros, array $macro_values) {
        ...
        $rows = (new Query())->select(['f.triggerid','f.functionid','n.ip','n.dns','n.type','n.useip','n.port'])
            ->from(['f' => 'functions'])
            ->leftJoin(['i' => 'items'], 'f.itemid=i.itemid')
            ->leftJoin(['n' => 'interface'], 'i.hostid=n.hostid')
            ->where(['f.functionid' => array_keys($macros), 'n.main' => 1])
            ->all();
	...
	protected static function getItemsValueMaps(array $items): array {
		foreach ($items as &$item) {
			$item['valuemap'] = [];
            $item['valuemapid'] = (int)$item['valuemapid'];
		}
		unset($item);
        ...
        $db_mappings = (new Query())->select(['valuemapid', 'type', 'value', 'newvalue'])
	...
	protected function getItemValueMacrosByItemid(array $macros, array $macro_values) {
		...
		$history = Manager::History()->getLastValues($db_items, 1, timeUnitToSeconds(
			SettingsHelper::get(SettingsHelper::HISTORY_PERIOD)
		));
			...
			foreach ($keys as $key => $tokens) {
				foreach ($tokens as $token => $data) {
					...
					switch ($data['macro']) {
						...
						case 'ITEM.LOG.AGE':
							$macro_values[$key][$token] = prs_date2age($history[$itemid][0]['timestamp']);
							break;
	...
	protected function getItemMacros(array $macros, array $macro_values, array $triggers = [], array $options = []) {
        ...
        $functions = (new Query())->select('f.triggerid,f.functionid,i.itemid,i.name,i.value_type,i.units,i.valuemapid')
            ->from(['f' => 'functions'])
            ->leftJoin(['i' => 'items'], 'f.itemid=i.itemid')
            ->leftJoin(['h' => 'hosts'], 'i.hostid=h.hostid')
            ->where(['f.functionid' => array_keys($macros)])
            ->all();
		...
		foreach ($functions as $function) {
			foreach ($macros[$function['functionid']] as $m => $tokens) {
				...
				switch ($m) {
					...
					case 'ITEM.LASTVALUE':
                        $history = HistoryHelper::getLatestHistory($function['itemid'], $function['value_type'], timeUnitToSeconds(
                            SettingsHelper::get(SettingsHelper::HISTORY_PERIOD)
                        ));
				...
				foreach ($tokens as $token) {
					if ($value !== null) {
						$macro_value = array_key_exists('macrofunc', $token)
							...
							: ItemHelper::formatHistoryValue($value, $function);
					}
					else {
			...
			foreach ($macros[$function['functionid']] as $m => $tokens) {
				...
				switch ($m) {
					...
					case 'ITEM.LOG.TIME':
						...
						break;
					case 'ITEM.LOG.AGE':
						$value = prs_date2age($history[$function['itemid']][0]['timestamp']);
						break;
					case 'ITEM.LOG.SOURCE':
	...
	protected function getHostMacros(array $macros, array $macro_values) {
        ...
        $rows = (new Query())->select(['f.triggerid','f.functionid','h.hostid','h.host','h.name'])
            ->from(['f' => 'functions'])
            ->leftJoin(['i' => 'items'], 'f.itemid=i.itemid')
            ->leftJoin(['h' => 'hosts'], 'i.hostid=h.hostid')
            ->where(['f.functionid' => array_keys($macros)])
	...
	protected static function getInterfaceMacrosByItemId(array $macros, array $macro_values): array {
		...
		foreach ($db_items as $itemid => $db_item) {
			if ($db_item['interfaceid'] != 0) {
				 ...
				 * Perseus agent, SNMP, JMX or IPMI interface of the host in this order of priority or to 'UNKNOWN' if
	...
	protected static function getHostNMacros(array $hosts_n, array $macro_values,
        ...
        $db_hosts = Hosts::find()->select(['host', 'name', 'description'])
            ->where(['hostid' => array_keys($hostids)])
            ->indexBy('hostid')
            ->asArray()->all();
	...
	protected function getUserMacros(array $data, bool $unset_undefined = false) {
		...
		if ($hostids) {
			do {
                ...
                $db_host_macros = Hostmacro::find()->select(['macro', 'value', 'type', 'hostid'])
				...
				$templateids = [];
                $db_host_templates = (new Query())->select(['ht.hostid', 'ht.templateid'])
		...
		if (!$all_macros_resolved) {
			// Global macros.
            $db_global_macros = Globalmacro::find()->select(['macro', 'value', 'type'])->asArray()->all();
	 ...
	 *     array (
	 *         0 => array (
	 *             '{USER.FULLNAME}' => 'Perseus Administrator',
	 *         ),
	 *         1 => array (
	 *             '{USER.NAME}' => 'Perseus',
	...
	public static function getMacroValue(array $macro): string {
		return ($macro['type'] == PRS_MACRO_TYPE_SECRET || $macro['type'] == PRS_MACRO_TYPE_VAULT)
			? PRS_SECRET_MASK
			: $macro['value'];
	}
