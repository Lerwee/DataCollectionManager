...
class CMacrosResolver extends CMacrosResolverGeneral
{
    ...
    private function resolveTexts(array $data) {
                ...
                if (array_key_exists('user_data', $matched_macros['macros'])
                    ...
                    foreach ($matched_macros['macros']['user_data'] as $macro) {
                        switch ($macro) {
                            ...
                            case '{USER.USERNAME}':
                                $macros[$hostid][$macro] = WebUser::$data['username'];
                                break;
        ...
        // Host macros.
        if ($host_hostids) {
            $dbHosts = (new Query())->select(['h.hostid','h.name','h.host'])
                ->from(['h' => 'hosts'])
                ->where(['h.hostid' => array_keys($host_hostids)])
                ->all();
        ...
        if ($this->isTypeAvailable('agentInterface') && $interface_hostids) {
            $dbInterfaces = (new Query())->select(['i.hostid','i.ip','i.dns','i.useip'])
                ->from(['i' => 'interface'])
                ->where(['i.main' => INTERFACE_PRIMARY])
                ->andWhere(['i.type' => INTERFACE_TYPE_AGENT])
                ->andWhere(['i.hostid' => array_keys($interface_hostids)])
                ->all();
        ...
        if ($this->isTypeAvailable('interfaceWithoutPort') && $interface_hostids) {
            ...
            $interfaces = (new Query())->select('i.hostid,i.interfaceid,i.ip,i.dns,i.useip,i.port,i.type,i.main')
                ->from(['i' => 'interface'])
                ->where(['i.main' => INTERFACE_PRIMARY])
                ->andWhere(['i.type' => self::interfacePriorities])
                ->all();
            ...
            if ($usermacros) {
                ...
                foreach ($usermacros as $triggerid => &$usermacros_data) {
                    if (array_key_exists($triggerid, $db_triggers)) {
                        $usermacros_data['hostids'] = prs_objectValues($db_triggers[$triggerid]['hosts'], 'hostid');
                    }
                }
    ...
    public function resolveTriggerExpressions(array $triggers, array $options)
    {
        ...
        if ($functionids) {
            ...
            if ($options['resolve_functionids']) {
                // Selecting functions.
                $rows = (new Query())->select(['f.functionid', 'f.itemid', 'f.name', 'f.parameter'])
                    ->from(['f' => 'functions'])
                    ->where(['f.functionid' => $functionids])
                    ->all();
                ...
                if ($itemids) {
                    if ($options['html']) {
                        $rows = (new Query())->select(['i.itemid', 'i.hostid', 'i.key_', 'i.type', 'i.flags', 'i.status', 'ir.state', 'id.parent_itemid'])
                            ->from(['i' => 'items'])
                            ->leftJoin(['ir' => 'item_rtdata'], 'i.itemid=ir.itemid')
                            ->leftJoin(['id' => 'item_discovery'], 'i.itemid=id.itemid')
                            ->where(['i.itemid' => array_keys($itemids)])
                            ->all();
                    } else {
                        $rows = (new Query())->select(['i.itemid', 'i.hostid', 'i.key_'])
                            ->from(['i' => 'items'])
                            ->where(['i.itemid' => array_keys($itemids)])
                            ->all();
                    }
                ...
                // Selecting hosts.
                if ($hostids) {
                    $rows = (new Query())->select(['h.hostid', 'h.host'])
                        ->from(['h' => 'hosts'])
                        ->where(['h.hostid' => array_keys($hostids)])
                        ->all();
                ...
                // Selecting functions.
                $rows = (new Query())->select(['f.functionid', 'i.hostid'])
                    ->from(['f' => 'functions', 'i' => 'items'])
                    ->where('f.itemid=i.itemid')
                    ->andWhere(['f.functionid' => $functionids])
                    ->all();
        ...
        foreach ($triggers as $key => $trigger) {
            foreach ($options['sources'] as $source) {
                ...
                foreach ($tokens as $token) {
                    ...
                    switch ($token['type']) {
                        case CExpressionParserResult::TOKEN_TYPE_MATH_FUNCTION:
                            $expression[] = $token['data']['function'] . '(';
                ...
                $triggers[$key][$source] = implode('', $expression);
            }
        }
     ...
     *   For example:
     *     {$last(/ /key)} => {$last(/Perseus server/key)}
        ...
        if ($itemids) {
            ...
            if ($interface_macros) {
                ...
                if ($hostids) {
                    $interfaces = (new Query())->select('i.hostid,i.interfaceid,i.ip,i.dns,i.useip,i.port,i.type,i.main')
                        ->from(['i' => 'interface'])
                        ->where(['i.hostid' => array_keys($hostids)])
                        ->where(['i.type' => self::interfacePriorities])
                        ->all();

                    $interfaces = CMacrosResolverHelper::resolveHostInterfaces($interfaces);
                    $interfaces = prs_toHash($interfaces, 'interfaceid');
     ...
     *                 'url' => 'http://perseus/ui/perseus.php?action=host.edit&hostid={HOST.ID}'
     *             ),
     *             61 => array(
     ...
     *         10084 => array (
     *             57 => array (
     *                 'confirmation' => 'Are you sure you want to edit Perseus server now?',
     *                 'url' => 'http://perseus/ui/perseus.php?action=host.edit&hostid=10084'
     *             ),
     *             61 => array (
     *                 'confirmation' => 'Hello, Perseus Administrator! Execute script?'
     *             )
     *         )
     *     )
     *
     ...
     *                 'url' => 'http://perseus/ui/triggers.php?context=host&filter_hostids[]={HOST.ID1}&filter_hostids[]={HOST.ID2}&filter_set=1'
     *             ),
     *             61 => array(
     *                 'confirmation' => 'Hello, {USER.FULLNAME}! Execute script?'
     *             )
     ...
     *         19 => array (
     *             57 => array (
     *                 'confirmation' => 'Responsible hosts Perseus server, Perseus PC! Navigate to triggers?',
     *                 'url' => 'http://perseus/ui/triggers.php?context=host&filter_hostids[]=10084&filter_hostids[]=10134&filter_set=1'
     *             ),
     *             61 => array (
     *                 'confirmation' => 'Hello, Perseus Administrator! Execute script?'
     *             )
