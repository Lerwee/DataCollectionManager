...
class C20ImportConverter extends CConverter {
	...
	public function convert($data) {
		$data['perseus_export']['version'] = '3.0';
		...
		if (array_key_exists('templates', $data['perseus_export'])) {
			$data['perseus_export']['templates'] = $this->convertTemplates($data['perseus_export']['templates']);
		}
		if (array_key_exists('graphs', $data['perseus_export'])) {
			$data['perseus_export']['graphs'] = $this->convertGraphs($data['perseus_export']['graphs']);
		}
		if (array_key_exists('triggers', $data['perseus_export'])) {
			$data['perseus_export']['triggers'] = $this->convertTriggers($data['perseus_export']['triggers']);
		}
	...
	protected function convertScreenItems(array $screen_items) {
		foreach ($screen_items as &$screen_item) {
			...
			if (prs_is_int($screen_item['resourcetype'])) {
	...
	protected function convertGraphs(array $graphs) {
		foreach ($graphs as &$graph) {
			...
			if (prs_is_int($graph['ymin_type_1'])) {
			...
			if (prs_is_int($graph['ymax_type_1'])) {
