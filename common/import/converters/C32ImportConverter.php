...
class C32ImportConverter extends CConverter {
	...
	public function convert($data) {
		$data['perseus_export']['version'] = '3.4';

		if (array_key_exists('hosts', $data['perseus_export'])) {
			$data['perseus_export']['hosts'] = $this->convertHosts($data['perseus_export']['hosts']);
		}
		if (array_key_exists('templates', $data['perseus_export'])) {
			$data['perseus_export']['templates'] = $this->convertHosts($data['perseus_export']['templates']);
		}
		if (array_key_exists('maps', $data['perseus_export'])) {
			$data['perseus_export']['maps'] = $this->convertMaps($data['perseus_export']['maps']);
		}
	...
	protected function convertItems(array $items,array $default_fields) {
		foreach ($items as &$item) {
			...
			if ($item['data_type'] != ITEM_DATA_TYPE_DECIMAL) {
				switch ($item['data_type']) {
					case ITEM_DATA_TYPE_OCTAL:
						$type = PRS_PREPROC_OCT2DEC;
						break;
					case ITEM_DATA_TYPE_HEXADECIMAL:
						$type = PRS_PREPROC_HEX2DEC;
						break;
					case ITEM_DATA_TYPE_BOOLEAN:
						$type = PRS_PREPROC_BOOL2DEC;
						break;
				}
			...
			if ($item['delta'] == 1) {
				$item['preprocessing'][] = ['type' => (string) PRS_PREPROC_DELTA_SPEED, 'params' => ''];
			}
			elseif ($item['delta'] == 2) {
				$item['preprocessing'][] = ['type' => (string) PRS_PREPROC_DELTA_VALUE, 'params' => ''];
			}

			if ($item['multiplier'] == 1) {
				$item['preprocessing'][] = ['type' => (string) PRS_PREPROC_MULTIPLIER, 'params' => $item['formula']];
			}
			...
			$item['jmx_endpoint'] = ($item['type'] == ITEM_TYPE_JMX) ? PRS_DEFAULT_JMX_ENDPOINT : '';
	...
	protected function convertMapElements(array $selements) {
		foreach ($selements as &$selement) {
			if (prs_is_int($selement['elementtype'])) {
