...
use app\customs\zapi\common\import\converters\CConverter;
use app\customs\zapi\common\import\converters\C52CalculatedItemConverter;
use app\customs\zapi\common\import\converters\C52AggregateItemKeyConverter;
use app\customs\zapi\common\import\converters\C52TriggerExpressionConverter;
use app\customs\zapi\common\import\converters\C52EventNameConverter;
use app\customs\zapi\common\parsers\CExpressionParser;
use app\customs\zapi\common\xml\CXmlConstantName;
...
class C52ImportConverter extends CConverter {
	...
	public function convert(array $data): array {
		$data['perseus_export']['version'] = '5.4';

		if (array_key_exists('value_maps', $data['perseus_export'])) {
			/**
			 ...
			 */
			$data['perseus_export'] = self::convertValueMaps($data['perseus_export']);
		}

		if (array_key_exists('hosts', $data['perseus_export'])) {
			$data['perseus_export']['hosts'] = self::convertHosts($data['perseus_export']['hosts']);
		}

		if (array_key_exists('templates', $data['perseus_export'])) {
			$data['perseus_export']['templates'] = self::convertTemplates($data['perseus_export']['templates']);
		}

		if (array_key_exists('maps', $data['perseus_export'])) {
			$data['perseus_export']['maps'] = self::convertMaps($data['perseus_export']['maps']);
		}

		if (array_key_exists('triggers', $data['perseus_export'])) {
			$data['perseus_export']['triggers'] = self::convertTriggers($data['perseus_export']['triggers'], true);
		}

		if (array_key_exists('graphs', $data['perseus_export'])) {
			$data['perseus_export']['graphs'] = self::convertGraphs($data['perseus_export']['graphs']);
		}

		if (array_key_exists('groups', $data['perseus_export'])) {
			foreach ($data['perseus_export']['groups'] as &$group) {
	...
	private static function convertValueMaps(array $import): array {
		$valuemaps = prs_toHash($import['value_maps'], 'name');
