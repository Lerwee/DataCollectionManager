...
class CXmlValidator extends CXmlValidatorGeneral {
	...
	public function validate(array $data, string $path): array {
		$rules = ['type' => XML_ARRAY, 'rules' => [
			'perseus_export' => ['type' => XML_ARRAY | XML_REQUIRED, 'check_unexpected' => false, 'rules' => [
		...
		$version = $data['perseus_export']['version'];

		if (!$this->factory->hasObject($version)) {
			throw new Exception(
				_s('Invalid tag "%1$s": %2$s.', '/perseus_export/version', _('unsupported version number'))
			);
		}

		if (in_array($version, ['1.0', '2.0', '3.0', '3.2', '3.4', '4.0', '4.2', '4.4', '5.0', '5.2'])) {
			unset($data['perseus_export']['screens']);
		}

		$data['perseus_export'] = $this->factory->getObject($version)
			->setStrict($strict)
			->setPreview($is_preview)
			->validate($data['perseus_export'], '/perseus_export');
