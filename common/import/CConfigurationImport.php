...
use app\customs\zapi\common\import\importers\CHostImporter;
use app\customs\zapi\common\import\importers\CMapImporter;
use app\customs\zapi\common\import\importers\CTemplateDashboardImporter;
use app\customs\zapi\common\import\importers\CTemplateImporter;
use app\customs\zapi\common\parsers\CExpressionParser;
use app\customs\zapi\services\assist\BaseItemAssist;
...
class CConfigurationImport {
	...
	protected function processTemplateGroups(): void {
		...
		if ($this->options['template_groups']['updateExisting'] && $groups_to_update) {
			TemplateGroupService::instance()->update($groups_to_update);
		...
		if ($this->options['template_groups']['createMissing'] && $groups_to_create) {
			$result = TemplateGroupService::instance()->create($groups_to_create);
			if (!$result->isSuccess()) {
				throw new ValidateException($result->getErrcode(), $result->getErrmsg());
			}
			$created_groups = $result->getData();
	...
	protected function processHostGroups(): void {
		...
		if ($this->options['host_groups']['updateExisting'] && $groups_to_update) {
			HostGroupService::instance()->updateByInternal($groups_to_update);
		...
		if ($this->options['host_groups']['createMissing'] && $groups_to_create) {
			$result = HostGroupService::instance()->createByInternal($groups_to_create);
			$created_groups = $result->getData();
			foreach ($created_groups['groupids'] as $index => $groupid) {
	...
	protected function processItems(): void {
		...
		foreach ($this->getFormattedItems() as $host => $items) {
			...
			foreach ($order_tree[$host] as $item_key => $level) {
				...
				if (array_key_exists('interface_ref', $item) && $item['interface_ref']) {
					...
					if ($interfaceid === null) {
						throw new ValidateException(60750001, t('zapi', 'Cannot find interface "{dst_name}" used for item "{src_name}" on "{host}".', [
							'dst_name' => $item['interface_ref'], 'src_name' => $item['name'], 'host' => $host
						]));
					}
				...
				if (array_key_exists('valuemap', $item) && $item['valuemap']) {
					...
					if ($valuemapid === null) {
						throw new ValidateException(60750001, t('zapi', 'Cannot find value map "{dst_name}" used for item "{src_name}" on "{host}".',[
							'dst_name' => $item['valuemap']['name'],
							'src_name' => $item['name'],
							'host' => $host
						]));
					}
				...
				if ($item['type'] == ITEM_TYPE_DEPENDENT) {
					if (!array_key_exists('key', $item[$master_item_key])) {
						throw new ValidateException(60750001, t('zapi', 'Incorrect value for field "{attribute}", {error}.', [
							'attribute' => 'master_itemid',
		...
		foreach (array_keys($levels) as $level) {
			if ($this->options['items']['updateExisting'] && array_key_exists($level, $items_to_update)) {
				$this->updateItemsWithDependency([$items_to_update[$level]], $master_item_key, ItemAssist::instance());
			}
			if ($this->options['items']['createMissing'] && array_key_exists($level, $items_to_create)) {
				$this->createItemsWithDependency([$items_to_create[$level]], $master_item_key, ItemAssist::instance());
			}
		}
	 ...
	 * @param ItemAssist|ItemPrototypeAssist $api_service  Entity service which is capable to proceed with entity create.
	 *
	 * @throws Exception if entity master entity can not be resolved.
	 */
	protected function createItemsWithDependency(array $items_by_level, string $master_item_key, BaseItemAssist $api_service): void {
		foreach ($items_by_level as $items_to_create) {
			foreach ($items_to_create as &$item) {
				if (array_key_exists($master_item_key, $item)) {
					...
					if ($item['master_itemid'] === null) {
						throw new ValidateException(60750001, t('zapi', 'Incorrect value for field "{attribute}", {error}.', [
							'attribute' => 'master_itemid',
							'error' => t('zapi', 'value "{value}" not found', ['value' => $item[$master_item_key]['key']])
						]));
					}
					unset($item[$master_item_key]);
				}
			...
			$result = $api_service->create($items_to_create);
			if (!$result->isSuccess()) {
				throw new ValidateException($result->getErrcode(), $result->getErrmsg());
			}

			$created_items = $result->getData();
	 ...
	 *                                           level and value is array of entities for this level.
	 * @param string $master_item_key            Master entity array key in xml parsed data.
	 * @param ItemAssist|ItemPrototypeAssist $api_service  Entity service which is capable to proceed with entity update.
	 *
	 * @throws Exception if entity master entity can not be resolved.
	 */
	protected function updateItemsWithDependency(array $items_by_level, string $master_item_key, BaseItemAssist $api_service): void {
		foreach ($items_by_level as $items_to_update) {
			...
			$result = $api_service->update($items_to_update);
			if (!$result->isSuccess()) {
				throw new ValidateException($result->getErrcode(), $result->getErrmsg());
			}

			$updated_items = $result->getData();
	...
	protected function processDiscoveryRules(): void {
		...
		foreach ($discovery_rules_by_hosts as $host => $discovery_rules) {
			...
			foreach ($discovery_rules as $discovery_rule) {
				...
				if (array_key_exists('interface_ref', $discovery_rule) && $discovery_rule['interface_ref']) {
					...
					if ($interfaceid === null) {
						throw new ValidateException(60750001, t('zapi', 'Cannot find interface "{dst_name}" used for discovery rule "{src_name}" on "{host}".',[
							'dst_name' => $discovery_rule['interface_ref'],
							'src_name' => $discovery_rule['name'],
							'host' => $host
						]));
					}
						...
						foreach ($override['operations'] as &$operation) {
							if ($operation['operationobject'] == OPERATION_OBJECT_HOST_PROTOTYPE
								...
								foreach ($operation['optemplate'] as &$template) {
									...
									if ($templateid === null) {
										throw new ValidateException(60750001, t('zapi', 'Cannot find template "{dst_name}" for override "{name}" of discovery rule "{src_name}" on "{host}".',[
											'dst_name' => $template['name'],
											'name' => $override['name'],
											'src_name' => $discovery_rule['name'],
											'host' => $host
										]));
		...
		if ($this->options['discoveryRules']['createMissing'] && $discovery_rules_to_create) {
			$result = DiscoverRuleAssist::instance()->create($discovery_rules_to_create);
			if (!$result->isSuccess()) {
				throw new ValidateException($result->getErrcode(), $result->getErrmsg());
			}
		...
		if ($this->options['discoveryRules']['updateExisting'] && $discovery_rules_to_update) {
			$result = DiscoverRuleAssist::instance()->update($discovery_rules_to_update);
			if (!$result->isSuccess()) {
				throw new ValidateException($result->getErrcode(), $result->getErrmsg());
			}
			...
			foreach ($discovery_rules as $discovery_rule) {
				...
				foreach ($item_prototypes as $index => $level) {
					...
					if (array_key_exists('interface_ref', $item_prototype) && $item_prototype['interface_ref']) {
						...
						if ($interfaceid !== null) {
							...
							throw new ValidateException(60750001, t('zapi', 'Cannot find interface "{dst_name}" used for item prototype "{name}" of discovery rule "{src_name}" on "{host}".',[
								'dst_name' => $item_prototype['interface_ref'],
					...
					if (array_key_exists('valuemap', $item_prototype) && $item_prototype['valuemap']) {
						...
						if ($valuemapid === null) {
							throw new ValidateException(60750001, t('zapi', 'Cannot find value map "{dst_name}" used for item prototype "{name}" of discovery rule "{src_name}" on "{host}".',[
								'dst_name' => $item_prototype['valuemap']['name'],
								'name' => $item_prototype['name'],
								'src_name' => $discovery_rule['name'],
								'host' => $host
							]));
				...
				foreach ($discovery_rule['host_prototypes'] as $host_prototype) {
					...
					foreach ($host_prototype['group_links'] as $group_link) {
						...
						if ($groupid === null) {
							throw new ValidateException(60750001, t('zapi', 'Cannot find host group "{dst_name}" for host prototype "{name}" of discovery rule "{src_name}" on "{host}".',[
								'dst_name' => $group_link['group']['name'],
								'name' => $host_prototype['name'],
								'src_name' => $discovery_rule['name'],
								'host' => $host
							]));
						}
					...
					foreach ($host_prototype['templates'] as $template) {
						...
						if ($templateid === null) {
							throw new ValidateException(60750001, t('zapi', 'Cannot find template "{dst_name}" for host prototype "{name}" of discovery rule "{src_name}" on "{host}".',[
								'dst_name' => $template['name'],
								'name' => $host_prototype['name'],
								'src_name' => $discovery_rule['name'],
								'host' => $host
		...
		if ($host_prototypes_to_update) {
			HostPrototypeService::instance()->update($host_prototypes_to_update);
		}

		if ($host_prototypes_to_create) {
			HostPrototypeService::instance()->create($host_prototypes_to_create);
		}
			...
			foreach ($discovery_rules as $discovery_rule) {
				...
				foreach ($discovery_rule['graph_prototypes'] as $graph) {
					if ($graph['ymin_item_1']) {
						...
						if ($itemid === null) {
							throw new ValidateException(60750001, t('zapi', 'Cannot find item "{item}" on "{dst_name}" used as the Y axis MIN value for graph prototype "{graph}" of discovery rule "{rule}" on "{src_name}".', [
								'item' => $graph['ymin_item_1']['key'],
								'dst_name' => $graph['ymin_item_1']['host'],
								'graph' => $graph['name'],
								'rule' => $discovery_rule['name'],
								'src_name' => $host
							]));
						}
					...
					if ($graph['ymax_item_1']) {
						...
						if ($itemid === null) {
							throw new ValidateException(60750001, t('zapi', 'Cannot find item "{item}" on "{dst_name}" used as the Y axis MAX value for graph prototype "{graph}" of discovery rule "{rule}" on "{src_name}".', [
								'item' => $graph['ymax_item_1']['key'],
								'dst_name' => $graph['ymax_item_1']['host'],
								'graph' => $graph['name'],
								'rule' => $discovery_rule['name'],
								'src_name' => $host
							]));
					...
					foreach ($graph['gitems'] as &$item) {
						...
						if ($item['itemid'] === null) {
							throw new ValidateException(60750001, t('zapi', 'Cannot find item "{item}" on "{dst_name}" used in graph prototype "{graph}" of discovery rule "{rule}" on "{src_name}".', [
								'item' => $item['item']['key'],
								'dst_name' => $item['item']['host'],
								'graph' => $graph['name'],
								'rule' => $discovery_rule['name'],
								'src_name' => $host
							]));
		...
		if ($triggers_to_update) {
			$result = TriggerPrototypeAssist::instance()->update($triggers_to_update);
			$updated_triggers = $result->getData();
		...
		if ($triggers_to_create) {
			$result = TriggerPrototypeAssist::instance()->create($triggers_to_create);
			$created_triggers = $result->getData();
	...
	protected function processTriggerPrototypeDependencies(array $triggers): void {
		...
		foreach ($triggers as $trigger) {
			...
			foreach ($trigger['dependencies'] as $dependency) {
				...
				if ($dependent_triggerid === null) {
					throw new ValidateException(60750001, t('zapi', 'Cannot find item "{dst_name}" depends on trigger "{src_name}", which does not exist.', [
						'dst_name' => $trigger['description'],
						'src_name' => $dependency['name']
					]));
				}
		...
		if ($trigger_dependencies) {
			$result = TriggerPrototypeAssist::instance()->update($trigger_dependencies);
			// if (!$result->isSuccess()) {
			// 	throw new ValidateException($result->getErrcode(), $result->getErrmsg());
			// }
		}
	...
	protected function processHttpTests(): void {
		...
		if ($this->options['httptests']['updateExisting'] && $httptests_to_update) {
			$result = HttpTestService::instance()->update($httptests_to_update);
			if (!$result->isSuccess()) {
				throw new ValidateException($result->getErrcode(), $result->getErrmsg());
			}
		}
		...
		if ($this->options['httptests']['createMissing'] && $httptests_to_create) {
			$result = HttpTestService::instance()->create($httptests_to_create);
			if (!$result->isSuccess()) {
				throw new ValidateException($result->getErrcode(), $result->getErrmsg());
			}
		}
	...
	protected function processGraphs(): void {
		...
		foreach ($this->getFormattedGraphs() as $graph) {
			if ($graph['ymin_item_1']) {
				...
				if ($itemid === null) {
					throw new ValidateException(60750001, t('zapi', 'Cannot find item "{item}" on "{host}" used as the Y axis MIN value for graph "{graph}".', [
						'item' => $graph['ymax_item_1']['key'],
						'host' => $graph['ymax_item_1']['host'],
						'graph' => $graph['name']
					]));
				}
			...
			if ($graph['ymax_item_1']) {
				...
				if ($itemid === null) {
					throw new ValidateException(60750001, t('zapi', 'Cannot find item "{item}" on "{host}" used as the Y axis MAX value for graph "{graph}".', [
						'item' => $graph['ymax_item_1']['key'],
						'host' => $graph['ymax_item_1']['host'],
						'graph' => $graph['name']
					]));
			...
			foreach ($graph['gitems'] as &$item) {
				...
				if ($item['itemid'] === null) {
					throw new ValidateException(60750001, t('zapi', 'Cannot find item "{item}" on "{host}" used in graph "{graph}".', [
						'item' => $item['item']['key'],
						'host' => $item['item']['host'],
						'graph' => $graph['name']
					]));
	...
	protected function processTriggers(): void {
		...
		if ($triggers_to_update) {
			$result = TriggerAssist::instance()->update($triggers_to_update);
			if (!$result->isSuccess()) {
				throw new ValidateException($result->getErrcode(), $result->getErrmsg());
			}
		}
		...
		if ($triggers_to_create) {
			$result = TriggerAssist::instance()->create($triggers_to_create);
			if (!$result->isSuccess()) {
				throw new ValidateException($result->getErrcode(), $result->getErrmsg());
			}
		}
	...
	private function isTemplateTrigger(array $trigger): bool {
		...
		if ($expression_parser->parse($trigger['expression']) != CExpressionParser::PARSE_SUCCESS) {
			return false;
		}
		...
		if ($trigger['recovery_expression'] === ''
				|| $expression_parser->parse($trigger['recovery_expression']) != CExpressionParser::PARSE_SUCCESS) {
			return false;
		}
	...
	protected function processTriggerDependencies(array $triggers): void {
		...
		foreach ($triggers as $trigger) {
			...
			foreach ($trigger['dependencies'] as $dependency) {
				...
				if ($dependent_triggerid === null) {
					throw new ValidateException(60750001, t('zapi', 'Trigger "{dst_name}" depends on trigger "{src_name}", which does not exist.',[
						'dst_name' => $trigger['description'],
						'src_name' => $dependency['name']
					]));
				}
		...
		if ($trigger_dependencies) {
			$result = TriggerAssist::instance()->update($trigger_dependencies);
			if (!$result->isSuccess()) {
				throw new ValidateException($result->getErrcode(), $result->getErrmsg());
			}
		}
	...
	protected function processMediaTypes(): void {
		...
		$media_types_to_import = prs_toHash($media_types_to_import, 'name');
		...
		// ]);
		// $db_media_types = prs_toHash($db_media_types, 'name');
	...
	protected function deleteMissingItems(): void {
		...
		$db_itemids = ItemHelper::getItems([
			'output' => ['itemid'],
		...
		if ($items_to_delete) {
			ItemAssist::instance()->delete(array_keys($items_to_delete));
		}
	...
	protected function deleteMissingTriggers(): void {
		...
		$db_triggerids = TriggerHelper::getTriggers([
			'output' => [],
			'selectHosts' => ['hostid'],
			'hostids' => $processed_hostids,
			'filter' => ['flags' => PRS_FLAG_DISCOVERY_NORMAL],
		...
		if ($triggerids_to_delete) {
			TriggerAssist::instance()->delete($triggerids_to_delete);
		}
	...
	protected function deleteMissingPrototypes(array $discovery_rules_by_hosts): void {
		...
		$query->andWhere(['hd.parent_itemid' => array_keys($discovery_ruleids)]);

		$db_host_prototypes = $query->select(['h.hostid'])->indexBy('hostid')->column();

		$host_prototypes_to_delete = array_diff_key($db_host_prototypes, $host_prototypeids);

		if ($host_prototypes_to_delete) {
			HostPrototypeService::instance()->delete(array_keys($host_prototypes_to_delete));
		}

		$db_trigger_prototypes = TriggerHelper::getTriggers([
			'output' => [],
		...
		if ($trigger_prototypes_to_delete) {
			TriggerPrototypeAssist::instance()->delete(array_keys($trigger_prototypes_to_delete));
		}
		...
		$db_item_prototypes = ItemHelper::getItemPrototypes([
			'output' => [],
			'discoveryids' => array_keys($discovery_ruleids),
			'preservekeys' => true,
			'nopermissions' => true,
		...
		if ($item_prototypes_to_delete) {
			ItemPrototypeAssist::instance()->delete(array_keys($item_prototypes_to_delete));
		}
	...
	protected function deleteMissingHttpTests(): void {
		...
		$db_httptestids = HttpTestService::instance()->getHttpTestsByIds([], $processed_hostids);


		$httptestids_to_delete = array_diff_key($db_httptestids, $httptestids);

		if ($httptestids_to_delete) {
			HttpTestService::instance()->delete(array_keys($httptestids_to_delete));
		}
	...
	protected function deleteMissingDiscoveryRules(): void {
		...
		$db_discovery_ruleids = DiscoverRuleHelper::getDiscoverRules([
			'output' => ['itemid'],
			'hostids' => $processed_hostids,
			'inherited' => false,
			'preservekeys' => true,
		...
		if ($discovery_ruleids_to_delete) {
			DiscoverRuleAssist::instance()->delete(array_keys($discovery_ruleids_to_delete));
		}
	...
	protected function getEntitiesOrder(array $items_by_hosts, string $master_item_key,
		...
		if ($parent_item_keys) {
			...
			$db_items = ItemHelper::getItems($options + ['webitems' => true]);
			if ($get_prototypes) {
				$db_items += ItemHelper::getItemPrototypes($options);
			}

			$resolve_entity_keys = [];
			$itemid_to_item_key_by_hosts = [];

			for ($level = 0; $level < PRS_DEPENDENT_ITEM_MAX_LEVELS; $level++) {
			...
			if ($missing_master_itemids) {
				throw new ValidateException(60750001, t('zapi', 'Incorrect value for field "{attribute}", {error}.', [
					'attribute' => 'master_itemid',
					'error' => t('zapi', 'maximum number of dependency levels reached')
				]));
			}
			...
			foreach ($resolve_entity_keys as $item) {
				...
				if (!array_key_exists($item['host'], $itemid_to_item_key_by_hosts) ||
					...
					throw new ValidateException(60750001, t('zapi', 'Incorrect value for field "{attribute}", {error}.', [
						'attribute' => 'master_itemid',
						'error' => t('zapi', 'value "{value}" not found', ['value' => $master_itemid])
					]));
				}
				...
				while ($item && $item['type'] == ITEM_TYPE_DEPENDENT) {
					...
					if (array_key_exists($host_name, $resolved_masters_cache)
						...
						if (($item['type'] == ITEM_TYPE_DEPENDENT
							...
							throw new ValidateException(60750001, t('zapi', 'Incorrect value for field "{attribute}", {error}.', [
								'attribute' => 'master_itemid',
								'error' => t('zapi', 'circular item dependency is not allowed')
							]));
						}
						...
						throw new ValidateException(60750001, t('zapi', 'Incorrect value for field "{attribute}", {error}.', [
							'attribute' => 'master_itemid',
							'error' => t('zapi', 'value "{value}" not found', ['value' => $master_key])
						]));
					}
