...
use app\customs\zapi\common\parsers\CQueryParser;
...
class CHistFunctionValidator extends CValidator {
	...
	public function validate($token) {
		$invalid_param_messages = [
			'invalid first parameter in function "{function}"',
			'invalid second parameter in function "{function}"',
			'invalid third parameter in function "{function}"',
			'invalid fourth parameter in function "{function}"',
			'invalid fifth parameter in function "{function}"',
			'invalid sixth parameter in function "{function}"',
			'invalid seventh parameter in function "{function}"'
		];
		...
		foreach ($params_spec as $index => $param_spec) {
			...
			if ($index >= count($params)) {
				if ($required) {
                    $this->setError(t('zapi', 'mandatory parameter is missing in function "{function}"', ['function' => $token['data']['function']]));
			...
			if ($param['match'] === '') {
				if ($required) {
					$this->setError(t('zapi', $invalid_param_messages[$index], ['function' => $token['data']['function']]));
					return false;
				}
		...
		foreach ($rules as $rule) {
			switch ($rule['type']) {
				...
				case 'time':
					...
					$min = array_key_exists('min', $rule) ? $rule['min'] : PRS_MIN_INT32;
					$max = array_key_exists('max', $rule) ? $rule['max'] : PRS_MAX_INT32;
	...
	private static function validatePeriod(string $sec_num, string $time_shift, int $mode, array $options): bool {
		switch ($mode) {
			case CHistFunctionData::PERIOD_MODE_DEFAULT:
				...
				if ($sec !== null) {
					return ($sec > 0 && $sec <= PRS_MAX_INT32);
				}

				if (preg_match('/^#(?<num>\d+)$/', $sec_num, $matches) == 1) {
					return ($matches['num'] > 0 && $matches['num'] <= PRS_MAX_INT32);
				}
			...
			case CHistFunctionData::PERIOD_MODE_TREND:
				...
				if ($sec !== null) {
					return ($sec > 0 && $sec <= PRS_MAX_INT32 && $sec % SEC_PER_HOUR == 0);
				}
